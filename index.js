...
var _emscripten_get_main_loop_timing = (mode, value) => {
  if (mode) HEAP32[_asan_js_check_index(HEAP32, ((mode) >> 2), ___asan_storeN)] = MainLoop.timingMode;
  if (value) HEAP32[_asan_js_check_index(HEAP32, ((value) >> 2), ___asan_storeN)] = MainLoop.timingValue;
};
...
var fillOrientationChangeEventData = eventStruct => {
  ...
  HEAP32[_asan_js_check_index(HEAP32, ((eventStruct) >> 2), ___asan_storeN)] = orientationIndex;
  HEAP32[_asan_js_check_index(HEAP32, (((eventStruct) + (4)) >> 2), ___asan_storeN)] = orientationAngle;
};
...
var ASM_CONSTS = {
  307543808: () => {
    FS.syncfs(false, function(err) {
      if (err) console.error("Sync error:", err); else console.log("Data synced to IndexedDB");
    });
  },
  307543936: () => {},
  307543937: () => {},
  ...
  307544284: () => (navigator.mediaDevices === undefined) ? 0 : 1,
  307544343: ($0, $1, $2, $3, $4, $5, $6) => {
    const device = $0;
    const w = $1;
    const h = $2;
  ...
  307548863: () => Module["SDL3"].audioContext.sampleRate,
  307548914: ($0, $1, $2, $3) => {
    var SDL3 = Module["SDL3"];
      ...
      var makePointerEventCStruct = function(event) {
        ...
        if (event.pointerType == "pen") {
          ...
          if (ptr != 0) {
            ...
            var idx = ptr >> 2;
            HEAP32[_asan_js_check_index(HEAP32, idx++, ___asan_storeN)] = event.pointerId;
            HEAP32[_asan_js_check_index(HEAP32, idx++, ___asan_storeN)] = (typeof (event.button) !== "undefined") ? event.button : -1;
            HEAP32[_asan_js_check_index(HEAP32, idx++, ___asan_storeN)] = event.buttons;
            HEAPF32[_asan_js_check_index(HEAPF32, idx++, ___asan_storeN)] = event.movementX;
            HEAPF32[_asan_js_check_index(HEAPF32, idx++, ___asan_storeN)] = event.movementY;
            HEAPF32[_asan_js_check_index(HEAPF32, idx++, ___asan_storeN)] = event.clientX - rect.left;
            HEAPF32[_asan_js_check_index(HEAPF32, idx++, ___asan_storeN)] = event.clientY - rect.top;
            HEAPF32[_asan_js_check_index(HEAPF32, idx++, ___asan_storeN)] = event.pressure;
            HEAPF32[_asan_js_check_index(HEAPF32, idx++, ___asan_storeN)] = event.tangentialPressure;
            HEAPF32[_asan_js_check_index(HEAPF32, idx++, ___asan_storeN)] = event.tiltX;
            HEAPF32[_asan_js_check_index(HEAPF32, idx++, ___asan_storeN)] = event.tiltY;
            HEAPF32[_asan_js_check_index(HEAPF32, idx++, ___asan_storeN)] = event.twist;
          }
        }
        return ptr;
      };
    ...
    if (target) {
      ...
      var makeDropEventCStruct = function(event) {
        ...
        if (ptr != 0) {
          var idx = ptr >> 2;
          var rect = target.getBoundingClientRect();
          HEAP32[_asan_js_check_index(HEAP32, idx++, ___asan_storeN)] = event.clientX - rect.left;
          HEAP32[_asan_js_check_index(HEAP32, idx++, ___asan_storeN)] = event.clientY - rect.top;
        }
        return ptr;
      };
  ...
  307562840: () => window.innerWidth,
  307562870: () => window.innerHeight,
  307562901: $0 => {
...
var wasmSourceMapFile = locateFile("index.wasm.map");
